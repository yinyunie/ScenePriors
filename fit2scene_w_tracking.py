...
def parse_args():
    parser = argparse.ArgumentParser(description="Overfit a single 3D Front scene.")
    parser.add_argument("--data_path", type=str,
                        default='/home/ynie/Projects/SceneSynthesis/datasets/3D-Front/3D-FRONT_samples/bed',
                        help="Give the absolute path of all view data.")
    parser.add_argument("--room_uid", type=str,
                        default='6a0e73bc-d0c4-4a38-bfb6-e083ce05ebe9_MasterBedroom-2679',
...
label_names = ['void', 'armchair', 'bookshelf', 'cabinet', 'ceiling_lamp', 'chair', 'children_cabinet', 'coffee_table', 'desk', 'double_bed', 'dressing_chair', 'dressing_table', 'kids_bed', 'nightstand', 'pendant_lamp', 'shelf', 'single_bed', 'sofa', 'stool', 'table', 'tv_stand', 'wardrobe']
n_classes = len(label_names)
cam_K = np.load('/home/ynie/Projects/SceneSynthesis/datasets/3D-Front/3D-FRONT_renderings_improved_mat/cam_K.npy')
image_size = cam_K[:2, 2] * 2 + 1
...
class DiffRender(nn.Module):
    def __init__(self, device='cuda', n_views=10, downsample_ratio=4):
        ...
        raster_settings = RasterizationSettings(
            image_size=(int(image_size[1])//downsample_ratio, int(image_size[0])//downsample_ratio),
    ...
    def project_points(meshes, cam_Ts, cam_Ks, image_sizes):
        ...
        # # visualize
        # view_id = 0
        # points2d = proj_points.detach().cpu().numpy().astype(np.int32)[0, view_id]
        # image_size = (360, 480)
        # template = -np.ones(shape=(image_size[0], image_size[1]), dtype=np.int32)
        # for inst_id, points2d_per_inst in enumerate(points2d):
        #     template[points2d_per_inst[:, 1], points2d_per_inst[:, 0]] = inst_id
    ...
    def forward(self, box3ds, meshes, cam_Ts, cam_Ks, image_sizes, render_mask_tr=None, start_render=True):
        centers = box3ds[..., :3]
        sizes = box3ds[..., 3:6]
        classes = box3ds[..., 6:]
